...
static const int TABLE_DRAW_CHANNEL_NOCLIP = 2;                     // When
static const float TABLE_BORDER_SIZE                     = 1.0f;    // FIXME-TABLE
static const float TABLE_RESIZE_SEPARATOR_HALF_THICKNESS = 4.0f;    // Extend
static const float TABLE_RESIZE_SEPARATOR_FEEDBACK_TIMER = 0.06f;   // Delay
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (outer_window->SkipItems) // Consistent
        return false;
    ...
    if (instance_no > 0)
    {
        ...
        instance_id = GetIDWithSeed(instance_no, GetIDWithSeed("##Instances", NULL, id)); // Push
    }
    else
    {
        instance_id = id;
    ...
    PushOverrideID(id);
    if (instance_no > 0)
        PushOverrideID(instance_id); // FIXME
    ...
    table->InnerClipRect.ClipWith(table->WorkRect);     // We
    table->InnerClipRect.ClipWithFull(table->HostClipRect);
    table->InnerClipRect.Max.y = (flags & ImGuiTableFlags_NoHostExtendY) ? ImMin(table->InnerClipRect.Max.y, inner_window->WorkRect.Max.y) : inner_window->ClipRect.Max.y;

    table->RowPosY1 = table->RowPosY2 = table->WorkRect.Min.y; // This
    table->RowTextBaseline = 0.0f; // This
    table->RowCellPaddingY = 0.0f;
    table->FreezeRowsRequest = table->FreezeRowsCount = 0; // This
    ...
    outer_window->DC.NavIsScrollPushableX = false; // Shortcut
        ...
        for (int n = 0; n < columns_count; n++)
        {
            ...
            else
            {
                ...
                *column = ImGuiTableColumn();
                ...
                column->IsPreserveWidthAuto = true; // Preserve
    ...
    // FIXME-DPI
    // This
    const float new_ref_scale_unit = g.FontSize; // g.Font-
...
static void TableSetupColumnFlags(ImGuiTable* table, ImGuiTableColumn* column, ImGuiTableColumnFlags flags_in)
{
    ...
    {
        IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiTableColumnFlags_WidthMask_)); // Check
    }
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    table->MinColumnWidth = ImMax(1.0f, g.Style.FramePadding.x * 1.0f); // g.Style.ColumnsMinSpacing

    // Part
    // Process
    int count_fixed = 0;                // Number
    int count_stretch = 0;              // Number
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if (start_auto_fit)
            column->AutoFitQueue = column->CannotSkipItemsQueue = (1 << 3) - 1; // Fit
    ...
    // Part
    float sum_width_requests = 0.0f;    // Sum
    float stretch_sum_weights = 0.0f;   // Sum
    ...
    const float width_removed = (table->HasScrollbarYPrev && !table->InnerWindow->ScrollbarY) ? g.Style.ScrollbarSize : 0.0f; // To
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        column->NavLayerCurrent = (ImS8)(table->FreezeRowsCount > 0 ? ImGuiNavLayer_Menu : ImGuiNavLayer_Main); // Use
        ...
        column->WorkMaxX = column->MaxX - table->CellPaddingX - table->CellSpacingX2; // Expected
        column->ItemWidth = ImFloor(column->WidthGiven * 0.65f);
        column->ClipRect.Min.x = column->MinX;
        column->ClipRect.Min.y = work_rect.Min.y;
        column->ClipRect.Max.x = column->MaxX; // column-
        column->ClipRect.Max.y = FLT_MAX;
        column->ClipRect.ClipWithFull(host_clip_rect);
        ...
        column->IsVisibleY = true; // column-
        const bool is_visible = column->IsVisibleX; // column-
        if (is_visible)
    ...
    table->BorderX1 = table->InnerClipRect.Min.x;// table-
    table->BorderX2 = table->InnerClipRect.Max.x;// table-
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ...
    IM_ASSERT(rows >= 0 && rows < 128); // Arbitrary

    table->FreezeColumnsRequest = (table->Flags & ImGuiTableFlags_ScrollX) ? (ImGuiTableColumnIdx)ImMin(columns, table->ColumnsCount) : 0;
    table->FreezeColumnsCount = (table->InnerWindow->Scroll.x != 0.0f) ? table->FreezeColumnsRequest : 0;
    table->FreezeRowsRequest = (table->Flags & ImGuiTableFlags_ScrollY) ? (ImGuiTableColumnIdx)rows : 0;
    table->FreezeRowsCount = (table->InnerWindow->Scroll.y != 0.0f) ? table->FreezeRowsRequest : 0;
    table->IsUnfrozenRows = (table->FreezeRowsCount == 0); // Make
...
void ImGui::TableSetColumnEnabled(int column_n, bool enabled)
{
    ...
    if (!table)
        return;
    IM_ASSERT(table->Flags & ImGuiTableFlags_Hideable); // See
    if (column_n < 0)
        column_n = table->CurrentColumn;
...
ImGuiID ImGui::TableGetColumnResizeID(ImGuiTable* table, int column_n, int instance_no)
{
    ...
    return instance_id + 1 + column_n; // FIXME
}
...
void ImGui::TableBeginRow(ImGuiTable* table)
{
    ...
    table->RowIndentOffsetX = window->DC.Indent.x - table->HostIndentX; // Lock

    window->DC.PrevLineTextBaseOffset = 0.0f;
    window->DC.CursorPosPrevLine = ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y + table->RowCellPaddingY); // This
    window->DC.PrevLineSize = window->DC.CurrLineSize = ImVec2(0.0f, 0.0f); // This
    ...
    if (is_visible)
    {
        ...
        if (draw_cell_bg_color)
        {
            ...
            for (ImGuiTableCellData* cell_data = &table->RowCellData[0]; cell_data <= cell_data_end; cell_data++)
            {
                ...
                cell_bg_rect.Min.x = ImMax(cell_bg_rect.Min.x, column->ClipRect.Min.x);     // So
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    window->DC.CursorPosPrevLine.x = window->DC.CursorPos.x; // PrevLine.y
...
void ImGui::TableEndCell(ImGuiTable* table)
{
    ...
    float* p_max_pos_x;
    if (table->RowFlags & ImGuiTableRowFlags_Headers)
        p_max_pos_x = &column->ContentMaxXHeadersUsed;  // Useful
    else
...
void ImGui::TableSetColumnWidthAutoAll(ImGuiTable* table)
{
    ...
    {
        ImGuiTableColumn* column = &table->Columns[column_n];
        if (!column->IsEnabled && !(column->Flags & ImGuiTableColumnFlags_WidthStretch)) // Cannot
            continue;
        column->CannotSkipItemsQueue = (1 << 0);
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            if (src_channel->_CmdBuffer.Size > 0 && src_channel->_CmdBuffer.back().ElemCount == 0 && src_channel->_CmdBuffer.back().UserCallback == NULL) // Equivalent
            ...
            if (!(column->Flags & ImGuiTableColumnFlags_NoClip))
            {
                float content_max_x;
                if (!has_freeze_v)
                    content_max_x = ImMax(column->ContentMaxXUnfrozen, column->ContentMaxXHeadersUsed); // No
                else if (merge_group_sub_n == 0)
                    content_max_x = ImMax(column->ContentMaxXFrozen, column->ContentMaxXHeadersUsed);   // Row
                else
                    content_max_x = column->ContentMaxXUnfrozen;                                        // Row
    ...
    if (merge_group_mask != 0)
    {
        ...
        g.DrawChannelsTempMergeBuffer.resize(splitter->_Count - LEADING_DRAW_CHANNELS); // Use
        ...
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (column->NextEnabledColumn == -1 && !is_resizable)
                if ((table->Flags & ImGuiTableFlags_SizingMask_) != ImGuiTableFlags_SizingFixedSame || (table->Flags & ImGuiTableFlags_NoHostExtendX))
                    continue;
            if (column->MaxX <= column->ClipRect.Min.x) // FIXME-TABLE
                continue;
...
void ImGui::TableSortSpecsBuild(ImGuiTable* table)
{
    ...
    if (dirty)
    {
        ...
        table->SortSpecs.SpecsDirty = true; // Mark
        table->IsSortSpecsDirty = false; // Mark
    }
...
void ImGui::TableHeader(const char* label)
{
    ...
    ItemSize(ImVec2(0.0f, label_height)); // Don
    if (!ItemAdd(bb, id))
        return;
...
void ImGui::TableOpenContextMenu(int column_n)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    if (column_n == -1 && table->CurrentColumn != -1)   // When
        column_n = table->CurrentColumn;
    if (column_n == table->ColumnsCount)                // To
        column_n = -1;
...
void ImGui::TableDrawContextMenu(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_Resizable)
    {
        ...
        if (table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount && (table->Flags & ImGuiTableFlags_SizingMask_) != ImGuiTableFlags_SizingFixedSame)
            size_all_desc = LocalizeGetMsg(ImGuiLocKey_TableSizeAllFit);        // SizeAll
        else
            size_all_desc = LocalizeGetMsg(ImGuiLocKey_TableSizeAllDefault);    // SizeAll
...
void ImGui::TableResetSettings(ImGuiTable* table)
{
    ...
    table->IsSettingsRequestLoad = false;                   // Don
    table->SettingsLoadedFlags = ImGuiTableFlags_None;      // Mark
}
...
void ImGui::TableLoadSettings(ImGuiTable* table)
{
    ...
    if (table->SettingsOffset == -1)
    {
        ...
        if (settings == NULL)
            return;
        if (settings->ColumnsCount != table->ColumnsCount) // Allow
            table->IsSettingsDirty = true;
        table->SettingsOffset = g.SettingsTables.offset_from_ptr(settings);
    }
    else
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        ...
        buf->reserve(buf->size() + 30 + settings->ColumnsCount * 50); // ballpark
...
void ImGui::TableGcCompactTransientBuffers(ImGuiTable* table)
{
    ...
    table->IsSortSpecsDirty = true; // FIXME
...
{
    const bool is_active = (table->LastFrameActive >= GetFrameCount() - 2); // Note
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    IM_ASSERT(window->DC.CurrentColumns == NULL);   // Nested
    ...
    window->DC.NavIsScrollPushableX = false; // Shortcut
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_GrowParentContentsSize))
        window->DC.CursorMaxPos.x = columns->HostCursorMaxPosX;  // Restore
